...
const R = window.SIDERA_REGISTRY, S = window.SideraSchema, V = window.Sidera.viz, D = window.SideraDemo;
...
function renderGallery() {
  ...
  c.innerHTML += `<table class="tbl"><thead><tr><th>图元</th><th>中文</th><th>级</th><th>用于</th><th>实现要点</th><th>状态</th></tr></thead>
    <tbody>${lib.map(r => `<tr><td><code>${r[0]}</code></td><td>${r[1]}</td><td class="num">${r[2]}</td>
      ...
      <td>${window.Sidera.viz[r[0]] ? '<span class="chip g">已实现</span>' : '<span class="chip">待实现</span>'}</td></tr>`).join('')}</tbody></table>`;
  out.appendChild(c);
...
function renderRegistry() {
  ...
  c.appendChild(el('div', 'srcbar', `<span class="cap">SINGLE SOURCE OF TRUTH</span><span class="srct">
    ...
    对应 <code>src/sidera.registry.js</code>，可直接导出为 JSON 供后端使用。</span>`));
