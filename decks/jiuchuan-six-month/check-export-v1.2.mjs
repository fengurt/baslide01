...
fs.mkdirSync(root+'/tmp/pdfs/jiuchuan-v12',{recursive:true});
const browser=await chromium.launch({headless:true,executablePath:'/Applications/Google Chrome.app/Contents/MacOS/Google Chrome'});
const page=await browser.newPage({viewport:{width:1440,height:900}});
await page.goto(pathToFileURL(dir+'/v1.2.html').href);
...
const audit=await page.evaluate(()=>{
 ...
 let issues=[];
 slides.forEach((s,i)=>{
  for(const e of s.querySelectorAll('svg text')){const box=e.getBBox(),v=e.ownerSVGElement.viewBox.baseVal;if(!e.hasAttribute('transform')&&(box.x<v.x-2||box.y<v.y-2||box.x+box.width>v.x+v.width+2||box.y+box.height>v.y+v.height+2))issues.push({page:i+1,svgText:e.textContent,box:{x:box.x,y:box.y,w:box.width,h:box.height},view:{w:v.width,h:v.height}});}
...
fs.writeFileSync(root+'/tmp/pdfs/jiuchuan-v12/audit.json',JSON.stringify(audit,null,2));
console.log(JSON.stringify(audit.issues));
if(audit.slides!==78||audit.images.length||audit.issues.length)throw new Error('Layout check failed');
...
for(let start=0;start<78;start+=16){
 ...
 await page.pdf({path:root+'/tmp/pdfs/jiuchuan-v12/batch-'+start+'.pdf',printBackground:true,preferCSSPageSize:true});
}
await browser.close();


// ponytail: use 16-page batches while Chromium rejects this deck as one print job.
execFileSync('/Users/af/.cache/codex-runtimes/codex-primary-runtime/dependencies/python/bin/python3',['-c',`
from pypdf import PdfReader,PdfWriter
w=PdfWriter()
for i in range(0,78,16):
 r=PdfReader(f'tmp/pdfs/jiuchuan-v12/batch-{i}.pdf')
 assert len(r.pages)==min(16,78-i)
 w.append(r)
w.add_metadata({'/Title':'九川文化 · 双店诊断与六个月陪跑方案 V1.2','/Author':'侍天 TIANSIGHT'})
w.write('decks/jiuchuan-six-month/九川文化-双店诊断与六个月陪跑方案-V1.2.pdf')
`],{stdio:'inherit'});
