...
description: Generate and audit 2880×1620 HTML report slides with 侍天 TIANSIGHT v2.0 tokens, 4 L1 shells × 12 L2 jobs, data figures, semantic tables, hideable explain panels, font packs, and PDF quality gates. Use when the user asks for TIANSIGHT slides, 侍天报告页, F&B diagnosis / briefing / roadmap / dossier decks, or HTML slides that must carry SOURCE / GLOSSARY / CONCLUSION / CONFIDENCE.
---
...
9. Header is one line (logo + chip). Bar values stay inside the mark or sit outside in ink with a paper halo — never paper-colored type on `--sd-paper`. Lists stay compact. Statement claim fills a paper well; 2–4 依据 are peer cards. Leftover one-row / one-line pages are absorbed. `data-pack` follows content so a short claim is air and a 10-row roster is tight. Decision-critical copy, SVG labels, and raw tables meet the native and fitted-view font floors in `references/quality-checklist.md`; chart pages with eight or more raw-data rows use a balanced split or a separate roster, never smaller type.
...
Before authoring or restyling a substantial deck, inspect the relevant pages in `decks/stone-briefing`, `decks/stone-dossier`, and `decks/stone-roadmap`. Benchmark their information hierarchy and figure-to-copy ratio; do not copy their content.

- For generation and redesign, read [references/quality-checklist.md](references/quality-checklist.md). It is the source of truth for page classification, data visualization, semantic tables, typography, internal-field hygiene, and efficient PDF export.
- For proofreading, publishing, or a user request to “全局检查 / 审计 / 最后检查”, read [references/audit-checklist.md](references/audit-checklist.md) and complete every applicable gate before export.
- Run the complete HTML audit before the first PDF export. A shared renderer or token change requires all affected decks to be regenerated; a deck-local change requires only that deck to be re-exported.
